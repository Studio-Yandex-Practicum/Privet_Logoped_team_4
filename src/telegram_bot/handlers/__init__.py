from .admin_handler import router as admin_router  # noqa
from .admin_links_handler import router as admin_links_router  # noqa
from .admin_users_handler import router as admin_users_router  # noqa
from .admin_promocodes_handler import router as admin_promocodes_router  # noqa
from .admin_upload_handler import router as admin_upload_router  # noqa
from .faq_handler import router as faq_router  # noqa
from .file_handler import router as file_router  # noqa
from .parent_handler import router as parent_router  # noqa
from .start_handler import router as start_router  # noqa
from .promocode_handler import router as promocode_router  #noqa
from .therapist_handler import router as therapist_router  # noqa