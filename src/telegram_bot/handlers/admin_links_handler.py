...
async def get_link_name(message: Message, state: FSMContext):
    ...
    if message.text == "Отмена":
        await message.answer("Отмена добавления ссылки.", reply_markup=kb.links)
        await state.set_state(AdminStates.links)
    else:
        await state.update_data(waiting_link_name=message.text)
        await message.answer("Выберите тип ссылки:", reply_markup=kb.links_types)
        ...
        try:
            async with async_session() as session:
                new_link = insert(Link).values(
                    ...
                    link_name=link_name,
                    link_type=link_type,
                    to_role=link_to_role,
                )
                await session.execute(new_link)
                await session.commit()
        except Exception:
            await message.answer("Попробуйте еще раз.", reply_markup=kb.links)
        else:
            await message.answer(
                f'Ссылка "{link_name}" успешно добавлена.', reply_markup=kb.links
            )
    await state.set_state(AdminStates.links)
...
async def delete_link(message: Message, state: FSMContext):
    ...
    """
    if message.text == "Отмена":
        await message.answer("Отмена удаления ссылки.", reply_markup=kb.links)
    else:
        try:
            ...
            try:
                ...
                await message.answer("Ссылка успешно удалена.", reply_markup=kb.links)
    await state.set_state(AdminStates.links)
