...
async def add_data(session):
    ...
    user1 = TGUser(user_id=1, role=RoleType.PARENT)
    user2 = VKUser(user_id=2, role=RoleType.PARENT)
    user3 = VKUser(user_id=3, role=RoleType.SPEECH_THERAPIST)
    session.add_all([user1, user2, user3])

    some_link1 = Link(
        link="https://example.com/some_link1",
        link_name="some_link1",
        link_type=LinkType.URL,
        to_role=RoleType.PARENT,
    )
    some_link2 = Link(
        link="https://example.com/some_link2",
        link_name="some_link2",
        link_type=LinkType.URL,
        to_role=RoleType.SPEECH_THERAPIST,
    )
    some_link3 = Link(
        link="home/files/filepath1", link_name="filepath1", link_type=LinkType.FILEPATH
    )
    session.add_all([some_link1, some_link2, some_link3])

    promocode1 = PromoCode(promocode="promo1", file_path="/home/promocodes/1")
    promocode2 = PromoCode(promocode="promo2", file_path="/home/promocodes/2")
...
async def fetch_data(session):
    ...
    result = await session.execute(select(VKUser))
    users = result.scalars().all()
    for user in users:
        print(
    ...
    for promocode in promocodes:
        print(f"PromoCode: {promocode.promocode}, " f"File Path: {promocode.file_path}")
