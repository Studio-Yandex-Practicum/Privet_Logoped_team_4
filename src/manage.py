...
async def promote_to_admin(user_id, platform):
    ...
    async with async_session() as session:
        async with session.begin():
            new_admin = (
                insert(model)
                .values(user_id=user_id, role=RoleType.SPEECH_THERAPIST, is_admin=1)
                .on_conflict_do_update(
                    constraint=model.__table__.primary_key, set_={model.is_admin: 1}
                )
            )
...
async def demote_admin(user_id, platform):
    ...
    async with async_session() as session:
        async with session.begin():
            delete_admin = (
                update(model).where(model.user_id == user_id).values(is_admin=0)
            )
